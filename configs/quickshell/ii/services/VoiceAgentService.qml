...
Singleton {
    ...
    function _launchProcesses() {
        ...
        // Pass explicit audio device names from the Audio service (tracks sidebar selection)
        var sourceName = Audio.source?.name ?? ""
        var sinkName = Audio.sink?.name ?? ""
        if (sourceName) {
            cmdParts.push("--audio-source=" + sourceName)
        }
        if (sinkName) {
            cmdParts.push("--audio-sink=" + sinkName)
        }
    ...
    function _startPlayback() {
        ...
        var sinkTarget = Audio.sink?.name ?? "@DEFAULT_SINK@"
        playbackProcess.command = ["bash", "-c",
            ...
            + "\" | pw-play --target=" + sinkTarget + " --format=s16 --rate=" + rate + " --channels=1 -"
        ]
